...
CHANGE FROM ORIGINAL: Gold output filenames are now namespaced by run_id
(e.g. "gold_sales_8f2a91c3.parquet" instead of "gold_sales.parquet"). Without
this, two different runs producing a target table with the same name (a very
likely occurrence — "gold_sales", "gold_summary" etc. are generic names an
LLM naturally reaches for) would silently overwrite each other's Gold Parquet
on disk. That's fine as long as nothing needs to look at old runs — but the
whole point of run history + follow-up chat on past runs is to look at old
runs' Gold data after newer runs have executed. Without namespacing, clicking
into an old run and asking a question would silently query a different run's
(newer) data with no error, which is worse than a crash.
"""
...
def _apply_gold_rules(
    ...
    output_paths = []
    # Short, stable per-run suffix for output filenames — see module docstring.
    run_suffix = run_id[:8] if run_id else "norun"
    ...
    for target_table_name, table_rules in target_tables:
        ...
        # Namespaced by run_id so distinct runs never overwrite each other's
        # Gold Parquet on disk, even when they produce a target table with
        # the same name (e.g. two runs both naming a table "gold_sales").
        output_filename = f"{target_table_name}_{run_suffix}.parquet"
        output_path = str(GOLD_DIR / output_filename)
        df.to_parquet(output_path, index=False)
        output_paths.append(output_path)

        print(f"[GOLD] Created {target_table_name}: {df.shape[0]} rows x {df.shape[1]} columns -> {output_path}")
        audit.log(
...
def execute_gold(
    ...
    Returns:
        list[str]: Gold Parquet output file paths, namespaced by run_id.
    """
